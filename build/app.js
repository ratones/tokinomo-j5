...
var Arduino = function () {
    function Arduino() {
        ...
        player.addEventListener('ended', function () {
            ...
            self.isMotorHome().then(function () {
...
var _status = require('./selftest/status');
...
if (!deviceID) {
    var newID = prompt('Please provide the device ID:');
    _settings2.default.persistKey('deviceID', newID);
    deviceID = newID;
}
...
function loadFiles() {
    client.downloadMelodies(deviceID).then(function () {
...
var HttpClient = function () {
    ...
    _createClass(HttpClient, [{
        ...
        value: function postFormData(data) {
            ...
            return new Promise(function (resolve, reject) {
                $.ajax({
                    url: _this2.baseUrl + '/api/index.php/utils/updates',
                    data: data,
                    type: 'POST',
        ...
        key: 'unpackUpdate',
        value: function unpackUpdate() {
            var self = this;
            var filename = path.join(os.tmpdir(), 'test.zip');
            exec('"' + path.resolve(process.cwd(), 'tools/unzip.exe') + '" -u -o "' + filename + '" -d "' + path.join(process.cwd(), '') + '" > NUL', function (err) {
                if (err) {
                    // self.statusEl = null;
...
var SelfTest = function () {
    function SelfTest() {
        ...
        _classCallCheck(this, SelfTest);
    }

    _createClass(SelfTest, [{
        key: 'checkIntegrity',
        value: function checkIntegrity() {
            var _this = this;

            var self = this;
            this.cam = new _camhandler2.default({
                image: true,
                debug: true
            });
            return new Promise(function (resolve, reject) {
                        ...
                        if (!err) {
                            ...
                            fs.writeFile("C:/Device/compare.png", base64Data, 'base64', function (err) {
                                ...
                                function doneReading() {
                                    ...
                                    var px = pixelmatch(img1.data, img2.data, diff.data, img1.width, img1.height, { threshold: 0.8 });
                                    if (px < 10) resolve(true);else resolve(false);
        ...
        key: 'onLevelChange',
        value: function onLevelChange(time) {
        ...
        value: function volumeAudioProcess(event) {
...
var CamHandler = function () {
    ...
    _createClass(CamHandler, [{
        key: 'setVideo',
        value: function setVideo(options) {
            ...
            this.player = videojs("myVideo", {
                ...
                // print version information at startup
                videojs.log('Using video.js', videojs.VERSION, 'with videojs-record', videojs.getPluginVersion('record'), '+ videojs-wavesurfer', videojs.getPluginVersion('wavesurfer'), 'and recordrtc', RecordRTC.version);
            });
        }
    }, {
        key: 'start',
        ...
        value: function snapshot() {
            ...
            return new Promise(function (resolve, reject) {
                ...
                _this.player.on('deviceReady', function () {
                    _this.player.record().start();
                });
                _this.player.on('finishRecord', function () {
                    resolve(_this.player.recordedData);
                    _this.player.off('deviceReady');
                    _this.player.off('finishedRecord');
                    _this.player.off('error');
                });
        ...
        value: function record() {
            ...
            return new Promise(function (resolve, reject) {
                _this2.player.record().getDevice();
                _this2.player.on('deviceReady', function () {
                    _this2.player.record().start();
                });
                _this2.player.on('finishRecord', function () {
                    resolve(_this2.player.recordedData);
